<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="90px"
			>
			<el-form-item class="add-item" label="提车编号" prop="tichebianhao">
				<el-input v-model="ruleForm.tichebianhao" placeholder="提车编号" disabled></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="提车时间" prop="ticheshijian">
				<el-date-picker
					:disabled="true  ||ro.ticheshijian"
					value-format="yyyy-MM-dd HH:mm:ss"
					v-model="ruleForm.ticheshijian" 
					type="datetime"
					placeholder="提车时间">
				</el-date-picker>
			</el-form-item>
...
<script>
	export default {
		...
		created() {
			...
			this.ruleForm.ticheshijian = this.getCurDateTime()
		},
		methods: {
			getMakeZero(s) {
				return s < 10 ? '0' + s : s;
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						if(o=='tichebianhao'){
							this.ruleForm.tichebianhao = obj[o];
							this.ro.tichebianhao = true;
							continue;
						}
						if(o=='qichemingcheng'){
						...
						if(o=='ticheshijian'){
							this.ruleForm.ticheshijian = obj[o];
							this.ro.ticheshijian = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
			...
			// 多级联动参数
			info() {
				this.$http.get(`tichexinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			// 提交
			async onSubmit() {
				if(this.ruleForm.tichebianhao){
					this.ruleForm.tichebianhao = String(this.ruleForm.tichebianhao)
				}
				if(!this.ruleForm.id) {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`tichexinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
