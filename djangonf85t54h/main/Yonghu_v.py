    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            for user in datas:
                ...
                file_path2 = os.path.join(os.getcwd(), "templates/upload", face2[7:])
    ...
    if request.method in ["POST", "GET"]:
        ...
        if os.path.exists(json_filename) == True:
            ...
            for item in req_dict['yColumnNameMul'].split(','):
                ...
                if timeStatType == '季':
                    sql = "SELECT CONCAT(YEAR(MIN({0})), '-Q', QUARTER(MIN({0}))) {0}, sum({1}) total FROM yonghu {2} GROUP BY YEAR({0}), QUARTER({0}) LIMIT 10".format(xColumnName, item, where)
